...
class KickCog(commands.Cog):
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: str) -> None:
        ...
        if staff_role in staff_user.roles:
            ...
            try:
                embed = discord.Embed(title="Mort", description=f"{member.mention}, you were kicked from {interaction.guild.name} for {reason} at <t:{timestamp}:f>.", color=discord.Color.from_str(embed_color))
                await member.send(embed=embed)

                await kick_member(interaction, member, reason)

                embed = discord.Embed(title="Mort", description=f"Successfully kicked {member.mention}!", color=discord.Color.from_str(embed_color))
            ...
            embed = discord.Embed(title="Mort", description=f"**{member.mention}** was kicked by {interaction.user} for **{reason}**.", color=discord.Color.from_str(embed_color))
    ...
    async def ikick(self, interaction: discord.Interaction, name: Optional[str], uuid: Optional[str], reason: str) -> None:
        ...
        if staff_role in staff_user.roles:
            ...
            async with API() as api:
                ...
                if response:
                    embed = discord.Embed(title="Mort", description=f"Successfully kicked {name}!", color=discord.Color.from_str(embed_color))
                    await interaction.edit_original_response(embed=embed)
                else:
